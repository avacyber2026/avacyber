      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            style={panelStyle}
            className="flex flex-col overflow-hidden rounded-xl border py-2 shadow-xl border-gray-200/80 bg-white/98 text-gray-900 backdrop-blur-md dark:border-white/12 dark:bg-[#1e211e]/98 dark:text-gray-100 ring-1 ring-black/[0.04] dark:ring-white/[0.06]"
          >
            <div className="px-2 pb-2 shrink-0 border-b border-gray-200/70 dark:border-white/10">
              <div className="relative">
                <FiSearch className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500 pointer-events-none" size={16} />
                <input
                  ref={searchRef}
                  type="text"
                  value={query}
            ...
            <div className="overflow-y-auto overflow-x-hidden px-1.5 pt-1 min-h-0" style={{ maxHeight: maxResultsHeight }}>
              {loading ? (
                <div className="px-3 py-4 text-sm text-gray-500 text-center">…</div>
              ) : users.length === 0 ? (
                <div className="px-3 py-4 text-sm text-gray-500 text-center">{emptyListMessage}</div>
              ) : filtered.length === 0 ? (
                <div className="px-3 py-4 text-sm text-gray-500 text-center">{noMatchMessage}</div>
              ) : (
                filtered.map((u, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        isHi ? "bg-[#1F6A5C]/10 text-[#0d4f42] dark:bg-[#50BFA0]/12 dark:text-[#a5e8d4]" : "text-gray-800 dark:text-gray-200",
                        "hover:bg-gray-100/90 dark:hover:bg-white/[0.07]",
                      ].join(" ")}
                      onMouseEnter={() => setHighlight(idx)}
                      onClick={() => toggle(u.email)}
                    >
                      <input type="checkbox" readOnly checked={isSel} className="rounded border-gray-300 dark:border-white/30 text-brand-primary pointer-events-none" tabIndex={-1} />
                      {label}
                    </button>
                  );
                })
      ...
      >
        {selectedUsers.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-1 min-w-0 flex-1">
            {selectedUsers.map((u) => (
              <span key={u.email} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-gray-100 dark:bg-white/10 text-xs font-medium">
                {endUserPickerLabelOrFallback(u, u.email)}
                <button
                  type="button"
                  className="shrink-0 text-gray-500 hover:text-red-500 dark:hover:text-red-400"
        ...
        <motion.span animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.2 }} className="shrink-0 text-gray-500 dark:text-gray-400">
