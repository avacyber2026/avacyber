        ...
        {loading ? (
          ...
          <div className="max-w-4xl">
            <HStack className="flex-wrap gap-2 mb-3" spacing={2}>
              {statusBadgeText ? (
                <span className="px-2.5 py-1 rounded-lg text-xs font-medium border border-gray-200 dark:border-white/15 bg-white/80 dark:bg-white/5 text-gray-800 dark:text-gray-200">
                  {statusBadgeText}
                </span>
              ) : null}
              ...
              <span className="px-2.5 py-1 rounded-lg text-xs font-medium border border-gray-200 dark:border-white/15 text-gray-600 dark:text-gray-400">
                {ticket.type}
              </span>
            ...
            <Box className="rounded-xl border border-gray-200 dark:border-white/15 bg-white dark:bg-[#232522] p-5 mb-2">
              ...
              <Text fontSize="sm" className="text-gray-800 dark:text-gray-200 whitespace-pre-wrap">
                {ticket.text}
              </Text>
              {ticket.attachments && ticket.attachments.length > 0 ? (
                <>
                  <Divider className="my-4 border-gray-200 dark:border-white/10" />
                  <Text fontSize="sm" fontWeight={600} className="text-gray-500 dark:text-gray-400 mb-2">
                    ...
                    {ticket.attachments.map((a) => (
                      <li key={a.id}>
                        <a
                          ...
                          className="text-sm text-[#1F6A5C] dark:text-[#50BFA0] underline hover:opacity-90 break-all"
                          target="_blank"
              ...
              {(ticket.createdBy || ticket.assignedTo) && (
                <Text fontSize="xs" className="text-gray-500 mt-4">
                  {ticket.createdBy && (
                    <>
              ...
              <Box className="rounded-xl border border-gray-200 dark:border-white/15 bg-white dark:bg-[#232522] p-5 mt-4">
                ...
                <div className="space-y-2">
                  {ticket.recipients.map((rec) => (
                    <div key={rec.userEmail} className="flex items-center justify-between text-sm px-3 py-2 rounded-lg bg-gray-50 dark:bg-white/5">
                      <span className="text-gray-800 dark:text-gray-200">{rec.userEmail}</span>
                      <div className="flex items-center gap-2">
                        {rec.acknowledgedAt ? (
                          ...
                          <span className="px-2 py-0.5 rounded text-xs font-medium bg-gray-100 dark:bg-white/10 text-gray-500 dark:text-gray-400">
                            {t("tickets.pending")}
                          </span>
                        )}
                        {rec.replyText && (
                          <span className="text-xs text-gray-500 dark:text-gray-400 max-w-[200px] truncate" title={rec.replyText}>
                            {rec.replyText}
                          </span>
                        )}
                      </div>
